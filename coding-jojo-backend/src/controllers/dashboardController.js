  ...
  try {
    ...
    const courseProgress = user.enrolledCourses
      ...
      .map(enrollment => ({
        ...
        lastChapter: `Chapter ${Math.ceil((enrollment.progress / 100) * (enrollment.courseId.totalLessons || 10))}`
      }));
    ...
    const enrolledCourses = user.enrolledCourses.map(enrollment => ({
      ...
      progress: enrollment.progress,
      totalLessons: enrollment.courseId.totalLessons || 0,
      completedLessons: Math.floor((enrollment.courseId.totalLessons || 0) * enrollment.progress / 100),      thumbnail: enrollment.courseId.thumbnail || '/api/placeholder/300/200',
      duration: enrollment.courseId.duration 
        ? `${enrollment.courseId.duration.hours}h ${enrollment.courseId.duration.minutes}m`
        : 'Duration TBD',
      instructor: {
      ...
      lastChapter: enrollment.progress > 0 
        ? `Chapter ${Math.ceil((enrollment.progress / 100) * (enrollment.courseId.totalLessons || 10))}`
        : 'Introduction',
      lastAccessed: enrollment.enrolledAt || new Date().toISOString()
    }));
