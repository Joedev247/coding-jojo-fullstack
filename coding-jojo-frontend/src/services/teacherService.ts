...
class TeacherService {
  ...
  async login(credentials: { email: string; password: string }) {
    ...
    if (response.success && response.token) {
      ...
      localStorage.setItem('teacher_info', JSON.stringify(response.data));
      console.log('Stored token:', response.token.substring(0, 20) + '...'); // Debug log
      console.log('Stored teacher info:', response.data); // Debug log
    }
  ...
  async uploadVerificationDocuments(documents: {
    ...
    // Add certificates
    documents.certificates.forEach((file, index) => {
    ...
    if (documents.selfiePhoto) {
      formData.append('selfiePhoto', documents.selfiePhoto);
    }

    // Add additional verification info
    if (documents.dateOfBirth) {
      formData.append('dateOfBirth', documents.dateOfBirth);
    }
    if (documents.phoneNumber) {
      formData.append('phoneNumber', documents.phoneNumber);
    }

    try {
      const response = await fetch(`${this.baseURL}/verification/upload`, {
        method: 'POST',
  ...
  isAuthenticated(): boolean {
    const hasToken = !!(localStorage.getItem('teacher_token') || sessionStorage.getItem('teacher_token'));
    return hasToken && !this.isTokenExpired();
  }
